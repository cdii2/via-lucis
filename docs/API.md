...
- `GET /api/status` →
  ```json
  {
    ...
    "mode": "wait",                   // wait | follow | demo | accompaniment
                                      ...
                                      // (A166, §3-E item 8) — mode_ defaults
...
- `GET /api/songs` →
  ...
  `GET /api/status`. `parseOk` (A170, §3-E item 12 — the queued Wave C ask)
...
- `POST /api/songs/{name}/load` → `200` + status JSON. Parses the file,
  resets transport to 0, state `idle`. Typed failures (A160, §3-E item 2 —
...
## Shows (P2; docs/SHOW-FORMAT.md — A171, §3-E item 1)
